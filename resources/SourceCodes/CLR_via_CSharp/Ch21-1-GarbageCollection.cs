...
internal static class SafeHandleInterop
{
    ...
    private static extern IntPtr CreateEventBad(IntPtr pSecurityAttributes, Boolean manualReset, Boolean initialState,
        String name);

    // This prototype is robust
    [DllImport("Kernel32", CharSet = CharSet.Unicode, EntryPoint = "CreateEvent")]
    private static extern SafeWaitHandle CreateEventGood(IntPtr pSecurityAttributes, Boolean manualReset,
...
internal static class MemoryPressureAndHandleCollector
{
    ...
    private sealed class LimitedResource
    {
        ...
        void ShowThreshold()
        {
            var i = s_hc.GetType() // 获取类型
                .GetField("threshold", BindingFlags.Instance | BindingFlags.NonPublic) // 获取字段
                ?.GetValue(s_hc); // 获取字段的值
            Console.WriteLine("Threshold: " + i);
        }
    }
}
