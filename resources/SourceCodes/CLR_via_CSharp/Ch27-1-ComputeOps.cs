...
internal static class ExecutionContexts
{
    ...
    private static void SecurityExample()
    {
        ...
        AppDomain lowSecurityAppDomain = AppDomain.CreateDomain("LowSecurity", null,
            new AppDomainSetup() { ApplicationBase = AppDomain.CurrentDomain.BaseDirectory }, grantSet, null);
        ProxyType lowSecurityObject =
            (ProxyType)lowSecurityAppDomain.CreateInstanceAndUnwrap(typeof(ProxyType).Assembly.ToString(),
                typeof(ProxyType).FullName);
...
internal static class TaskDemo
{
    ...
    private static void UnobservedException()
    {
        ...
        {
            Task child = Task.Factory.StartNew(() => { throw new InvalidOperationException(); },
                TaskCreationOptions.AttachedToParent);
            // Child’s exception is observed, but not from its parent
            child.ContinueWith((task) =>
            {
                var _error = task.Exception;
            }, TaskContinuationOptions.OnlyOnFaulted);
        });
